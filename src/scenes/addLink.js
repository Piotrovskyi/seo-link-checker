...
const checkLink = require("../link-checker/checkLink");
const linksArrayToMessage = require("../link-checker/linksArrayToMessage");
const { cancelMenu, mainMenuButtons } = require("../bot/menus");
...
const newLinkWizard = new Scenes.WizardScene(
  ...
  (ctx) => {
    ...
    if (msg.includes(",")) {
      ...
      ctx.wizard.state.data.links = splittedByNewLine.map((el) => ({
        page: el,
      }));
  ...
  async (ctx) => {
    ...
    const checkedLinks = await Promise.all(
      ctx.wizard.state.data.links.map(checkLink)
    );
    ...
    ctx.reply(
      `Thank you for your replies, link added to your list`,
      mainMenuButtons(ctx)
    );
